...
public class JobsfarmercraftrecipetypeRecipeCategory implements IRecipeCategory<JobsfarmercraftrecipetypeRecipe> {
	public final static ResourceLocation UID = ResourceLocation.parse("palamod:jobsfarmercraftrecipetype");
	...
	public JobsfarmercraftrecipetypeRecipeCategory(IGuiHelper helper) {
		this.background = helper.createDrawable(TEXTURE, 0, 0, 164, 102);
		this.icon = helper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(PalamodModItems.FARMERJOBSITEM.get()));
	}

	@Override
	public mezz.jei.api.recipe.RecipeType<JobsfarmercraftrecipetypeRecipe> getRecipeType() {
		return PalamodModJeiPlugin.Jobsfarmercraftrecipetype_Type;
	}
	...
	@Override
	public void draw(JobsfarmercraftrecipetypeRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
		this.background.draw(guiGraphics);
	}

	@Override
	public void setRecipe(IRecipeLayoutBuilder builder, JobsfarmercraftrecipetypeRecipe recipe, IFocusGroup focuses) {
