...
public class Adminshopmobs2blazeScreen extends AbstractContainerScreen<Adminshopmobs2blazeMenu> {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (mouseX > leftPos + 151 && mouseX < leftPos + 167 && mouseY > topPos + 5 && mouseY < topPos + 21)
			guiGraphics.renderTooltip(font, Component.translatable("gui.palamod.adminshopmobs_2blaze.tooltip_close_the_gui"), mouseX, mouseY);
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		...
		guiGraphics.drawString(this.font,
				...
				AdshoppreviewamountblazeProcedure.execute(world, entity, guistate), 29, 74, -12829636, false);
		guiGraphics.drawString(this.font, Component.translatable("gui.palamod.adminshopmobs_2blaze.label_sell_price_120"), 25, 30, -12829636, false);
		guiGraphics.drawString(this.font, Component.translatable("gui.palamod.adminshopmobs_2blaze.label_buy_price_125"), 24, 44, -12829636, false);
	}
	...
	public void init() {
		...
		button_buy = Button.builder(Component.translatable("gui.palamod.adminshopmobs_2blaze.button_buy"), e -> {
			if (true) {
				PalamodMod.PACKET_HANDLER.sendToServer(new Adminshopmobs2blazeButtonMessage(0, x, y, z));
		...
		button_sell = Button.builder(Component.translatable("gui.palamod.adminshopmobs_2blaze.button_sell"), e -> {
			if (true) {
				PalamodMod.PACKET_HANDLER.sendToServer(new Adminshopmobs2blazeButtonMessage(1, x, y, z));
		...
		imagebutton_cross_no_button = new ImageButton(this.leftPos + 151, this.topPos + 5, 16, 16, 0, 0, 16, new ResourceLocation("palamod:textures/screens/atlas/imagebutton_cross_no_button.png"), 16, 32, e -> {
			if (true) {
				PalamodMod.PACKET_HANDLER.sendToServer(new Adminshopmobs2blazeButtonMessage(2, x, y, z));
				Adminshopmobs2blazeButtonMessage.handleButtonAction(entity, 2, x, y, z);
			}
		});
		guistate.put("button:imagebutton_cross_no_button", imagebutton_cross_no_button);
		this.addRenderableWidget(imagebutton_cross_no_button);
		imagebutton_arrow_adminshop = new ImageButton(this.leftPos + 131, this.topPos + 5, 16, 16, 0, 0, 16, new ResourceLocation("palamod:textures/screens/atlas/imagebutton_arrow_adminshop.png"), 16, 32, e -> {
			if (true) {
				PalamodMod.PACKET_HANDLER.sendToServer(new Adminshopmobs2blazeButtonMessage(3, x, y, z));
				Adminshopmobs2blazeButtonMessage.handleButtonAction(entity, 3, x, y, z);
			}
		});
		guistate.put("button:imagebutton_arrow_adminshop", imagebutton_arrow_adminshop);
		this.addRenderableWidget(imagebutton_arrow_adminshop);
		imagebutton_home_pixel_adminshop = new ImageButton(this.leftPos + 114, this.topPos + 4, 16, 16, 0, 0, 16, new ResourceLocation("palamod:textures/screens/atlas/imagebutton_home_pixel_adminshop.png"), 16, 32, e -> {
			if (true) {
				PalamodMod.PACKET_HANDLER.sendToServer(new Adminshopmobs2blazeButtonMessage(4, x, y, z));
