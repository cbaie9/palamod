...
public class JobsalchiguiScreen extends AbstractContainerScreen<JobsalchiguiMenu> implements PalamodModScreens.ScreenAccessor {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (mouseX > leftPos + 14 && mouseX < leftPos + 159 && mouseY > topPos + 26 && mouseY < topPos + 36) {
			String hoverText = GetxpalchitextProcedure.execute(entity);
			if (hoverText != null) {
	...
	protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int mouseX, int mouseY) {
		...
		guiGraphics.blit(RenderPipelines.GUI_TEXTURED, SPRITE_0, this.leftPos + 14, this.topPos + 26, Mth.clamp((int) JobsalchigetxpprogressbarProcedure.execute(entity) * 145, 0, 14355), 0, 145, 10, 14500, 10);
	}
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		guiGraphics.drawString(this.font, Component.translatable("gui.palamod.jobsalchigui.label_jobs_miner"), 9, 7, -1, false);
		guiGraphics.drawString(this.font, GetxpalchiProcedure.execute(entity), 13, 38, -1, false);
		guiGraphics.drawString(this.font, GetlevelalchiProcedure.execute(entity), 5, 63, -1, false);
	}
	...
	public void init() {
		...
		imagebutton_cross_no_button = new ImageButton(this.leftPos + 154, this.topPos + 5, 16, 16,
				new WidgetSprites(ResourceLocation.parse("palamod:textures/screens/cross_no_button.png"), ResourceLocation.parse("palamod:textures/screens/pointed_cross_no_button.png")), e -> {
					...
					if (true) {
						ClientPacketDistributor.sendToServer(new JobsalchiguiButtonMessage(1, x, y, z));
						JobsalchiguiButtonMessage.handleButtonAction(entity, 1, x, y, z);
					}
				}) {
			@Override
			public void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
