...
import palamod.world.inventory.JobshunterguiMenu;

import palamod.procedures.JobshuntergetxpprogressbarProcedure;
import palamod.procedures.GetxphuntertextProcedure;
import palamod.procedures.GetxphunterProcedure;
import palamod.procedures.GetlevelhunterProcedure;

import palamod.network.JobshunterguiButtonMessage;
...
public class JobshunterguiScreen extends AbstractContainerScreen<JobshunterguiMenu> implements PalamodModScreens.ScreenAccessor {
	...
	public JobshunterguiScreen(JobshunterguiMenu container, Inventory inventory, Component text) {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (mouseX > leftPos + 14 && mouseX < leftPos + 159 && mouseY > topPos + 26 && mouseY < topPos + 36) {
			if (GetxphuntertextProcedure.execute(entity) != null) {
				guiGraphics.renderComponentTooltip(font, Arrays.stream(GetxphuntertextProcedure.execute(entity).split("\\\\n")).map(Component::literal).collect(Collectors.toList()), mouseX, mouseY);
			}
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		...
		int yOffset = 0;
		yOffset = 0;
		for (Component actualComponent : Arrays.stream(Component.translatable("gui.palamod.jobshuntergui.label_jobs_miner").getString().split("\\\\n")).map(Component::literal).collect(Collectors.toList())) {
			guiGraphics.drawString(this.font, actualComponent, 9, 7 + yOffset, -1, false);
			heightPadding = 2;
			yOffset += this.font.lineHeight + heightPadding;
		}
		yOffset = 0;
		for (Component actualComponent : Arrays.stream(GetxphunterProcedure.execute(entity).split("\\\\n")).map(Component::literal).collect(Collectors.toList())) {
		...
		for (Component actualComponent : Arrays.stream(GetlevelhunterProcedure.execute(entity).split("\\\\n")).map(Component::literal).collect(Collectors.toList())) {
			guiGraphics.drawString(this.font, actualComponent, 89, 38 + yOffset, -1, false);
			heightPadding = 2;
			yOffset += this.font.lineHeight + heightPadding;
		}
	}
	...
	public void init() {
		super.init();
		button_how = Button.builder(Component.translatable("gui.palamod.jobshuntergui.button_how"), e -> {
			int x = JobshunterguiScreen.this.x;
			int y = JobshunterguiScreen.this.y;
			if (true) {
				PacketDistributor.sendToServer(new JobshunterguiButtonMessage(0, x, y, z));
				JobshunterguiButtonMessage.handleButtonAction(entity, 0, x, y, z);
			}
		}).bounds(this.leftPos + 23, this.topPos + 53, 40, 20).build();
		...
		imagebutton_cross_no_button = new ImageButton(this.leftPos + 154, this.topPos + 5, 16, 16,
				new WidgetSprites(ResourceLocation.parse("palamod:textures/screens/cross_no_button.png"), ResourceLocation.parse("palamod:textures/screens/pointed_cross_no_button.png")), e -> {
					...
					int y = JobshunterguiScreen.this.y;
					if (true) {
						PacketDistributor.sendToServer(new JobshunterguiButtonMessage(2, x, y, z));
						JobshunterguiButtonMessage.handleButtonAction(entity, 2, x, y, z);
					}
				}) {
			@Override
			public void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
