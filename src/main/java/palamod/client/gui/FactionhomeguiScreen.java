...
public class FactionhomeguiScreen extends AbstractContainerScreen<FactionhomeguiMenu> implements PalamodModScreens.ScreenAccessor {
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		...
		int yOffset = 0;
		yOffset = 0;
		for (Component actualComponent : Arrays.stream(Component.translatable("gui.palamod.factionhomegui.label_faction_home").getString().split("\\\\n")).map(Component::literal).collect(Collectors.toList())) {
			guiGraphics.drawString(this.font, actualComponent, 116, 3 + yOffset, -12829636, false);
			heightPadding = 2;
			yOffset += this.font.lineHeight + heightPadding;
		}
		yOffset = 0;
		for (Component actualComponent : Arrays.stream(Component.translatable("gui.palamod.factionhomegui.label_empty").getString().split("\\\\n")).map(Component::literal).collect(Collectors.toList())) {
			guiGraphics.drawString(this.font, actualComponent, 19, 71 + yOffset, -12829636, false);
			heightPadding = 2;
			yOffset += this.font.lineHeight + heightPadding;
		}
		yOffset = 0;
		for (Component actualComponent : Arrays.stream(Factionhomeguisubprocess1Procedure.execute(world, entity).split("\\\\n")).map(Component::literal).collect(Collectors.toList())) {
