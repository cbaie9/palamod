...
public class JobsminerguiScreen extends AbstractContainerScreen<JobsminerguiMenu> implements PalamodModScreens.ScreenAccessor {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (mouseX > leftPos + 14 && mouseX < leftPos + 159 && mouseY > topPos + 26 && mouseY < topPos + 36) {
			if (GetxpminertextProcedure.execute(entity) != null) {
				guiGraphics.renderComponentTooltip(font, Arrays.stream(GetxpminertextProcedure.execute(entity).split("\\\\n")).map(Component::literal).collect(Collectors.toList()), mouseX, mouseY);
			}
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		...
		int yOffset = 0;
		yOffset = 0;
		for (Component actualComponent : Arrays.stream(Component.translatable("gui.palamod.jobsminergui.label_jobs_miner").getString().split("\\\\n")).map(Component::literal).collect(Collectors.toList())) {
