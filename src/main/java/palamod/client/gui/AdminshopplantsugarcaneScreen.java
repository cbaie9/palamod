...
public class AdminshopplantsugarcaneScreen extends AbstractContainerScreen<AdminshopplantsugarcaneMenu> {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (mouseX > leftPos + 117 && mouseX < leftPos + 129 && mouseY > topPos + 6 && mouseY < topPos + 21)
			guiGraphics.renderTooltip(font, Component.translatable("gui.palamod.adminshopplantsugarcane.tooltip_return_to_the_adminshop_menu"), mouseX, mouseY);
	}
	...
	public void init() {
		...
		button_buy = Button.builder(Component.translatable("gui.palamod.adminshopplantsugarcane.button_buy"), e -> {
			if (true) {
				PalamodMod.PACKET_HANDLER.sendToServer(new AdminshopplantsugarcaneButtonMessage(0, x, y, z));
		...
		button_sell = Button.builder(Component.translatable("gui.palamod.adminshopplantsugarcane.button_sell"), e -> {
			if (true) {
				PalamodMod.PACKET_HANDLER.sendToServer(new AdminshopplantsugarcaneButtonMessage(1, x, y, z));
		...
		imagebutton_cross_no_button = new ImageButton(this.leftPos + 152, this.topPos + 5, 16, 16, 0, 0, 16, new ResourceLocation("palamod:textures/screens/atlas/imagebutton_cross_no_button.png"), 16, 32, e -> {
			if (true) {
				PalamodMod.PACKET_HANDLER.sendToServer(new AdminshopplantsugarcaneButtonMessage(2, x, y, z));
		...
		imagebutton_arrow_adminshop = new ImageButton(this.leftPos + 133, this.topPos + 5, 16, 16, 0, 0, 16, new ResourceLocation("palamod:textures/screens/atlas/imagebutton_arrow_adminshop.png"), 16, 32, e -> {
			if (true) {
				PalamodMod.PACKET_HANDLER.sendToServer(new AdminshopplantsugarcaneButtonMessage(3, x, y, z));
		...
		imagebutton_home_pixel_adminshop = new ImageButton(this.leftPos + 115, this.topPos + 5, 16, 16, 0, 0, 16, new ResourceLocation("palamod:textures/screens/atlas/imagebutton_home_pixel_adminshop.png"), 16, 32, e -> {
			if (true) {
				PalamodMod.PACKET_HANDLER.sendToServer(new AdminshopplantsugarcaneButtonMessage(4, x, y, z));
