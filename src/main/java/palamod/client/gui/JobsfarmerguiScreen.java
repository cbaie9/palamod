...
public class JobsfarmerguiScreen extends AbstractContainerScreen<JobsfarmerguiMenu> implements PalamodModScreens.ScreenAccessor {
	...
	public void extractRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTicks) {
		if (mouseX > leftPos + 14 && mouseX < leftPos + 159 && mouseY > topPos + 26 && mouseY < topPos + 36) {
			String hoverText = GetxpfarmertextProcedure.execute(entity);
			if (hoverText != null) {
	...
	public void extractBackground(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		guiGraphics.blit(RenderPipelines.GUI_TEXTURED, SPRITE_0, this.leftPos + 14, this.topPos + 26, Mth.clamp((int) JobsfarmergetxpprogressbarProcedure.execute(entity) * 145, 0, 14355), 0, 145, 10, 14500, 10);
	}
	...
	protected void extractLabels(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
		guiGraphics.text(this.font, Component.translatable("gui.palamod.jobsfarmergui.label_jobs_miner"), 9, 7, -1, false);
		guiGraphics.text(this.font, GetxpfarmerProcedure.execute(entity), 13, 38, -1, false);
		guiGraphics.text(this.font, GetlevelfarmerProcedure.execute(entity), 5, 63, -1, false);
	}
	...
	public void init() {
		...
		imagebutton_cross_no_button = new ImageButton(this.leftPos + 154, this.topPos + 5, 16, 16,
				new WidgetSprites(Identifier.parse("palamod:textures/screens/cross_no_button.png"), Identifier.parse("palamod:textures/screens/pointed_cross_no_button.png")), e -> {
					...
					int y = JobsfarmerguiScreen.this.y;
					if (true) {
						ClientPacketDistributor.sendToServer(new JobsfarmerguiButtonMessage(1, x, y, z));
