...
public class Clickergetnump6Procedure {
		...
		if (clicker_buildings.exists()) {
			{
				try {
					...
					input = main_clicker_buildings.get(("building_p" + Math.round(PalamodModVariables.MapVariables.get(world).clicker_page) + "_n6")).getAsDouble();
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
			for (int index1156 = 0; index1156 < 10; index1156++) {
				if (9 >= input) {
					break;
				} else {
					input = input - 10;
