...
public class KeytooltipProcedure {
	public static void execute(ItemStack itemstack) {
		...
		itemstack.set(DataComponents.CUSTOM_NAME, Component
				.literal((((("%1" + " - " + "%2 / %3").replace("%3", "" + max_stock)).replace("%2", "" + itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDoubleOr("key_stock", 0))).replace("%1", item_display))));
	}
}