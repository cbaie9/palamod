...
public class DamagestickprocessProcedure {
	public static void execute(LevelAccessor world, Entity entity, ItemStack itemstack) {
		...
		if (itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("cooldown") == 0 || getEntityGameType(entity) == GameType.CREATIVE) {
			for (int index1070 = 0; index1070 < 25; index1070++) {
				{
					Entity _shootFrom = entity;
					Level projectileLevel = _shootFrom.level();
					if (!projectileLevel.isClientSide()) {
						Projectile _entityToSpawn = initArrowProjectile(new Arrow(projectileLevel, 0, 0, 0, new Arrow(EntityType.ARROW, projectileLevel).getPickupItemStackOrigin(), createArrowWeaponItemStack(projectileLevel, (int) 7.5, (byte) 0)),
								entity, (float) 0.25, false, false, false, AbstractArrow.Pickup.CREATIVE_ONLY);
