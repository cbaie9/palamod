...
public class JobsminercraftfakewaterobsidianProcedure {
	public static void execute(Entity entity) {
		...
		{
			String _setval = "fake water when broken";
			entity.getCapability(PalamodModVariables.PLAYER_VARIABLES_CAPABILITY, null).ifPresent(capability -> {
		...
		{
			String _setval = "Lvl required : 42";
			entity.getCapability(PalamodModVariables.PLAYER_VARIABLES_CAPABILITY, null).ifPresent(capability -> {
				capability.lvl_required = _setval;
				capability.syncPlayerVariables(entity);
			});
		}
		if ((entity instanceof Player _playerHasItem ? _playerHasItem.getInventory().contains(new ItemStack(PalamodModItems.FAKEWTER_BUCKET.get())) : false) && new Object() {
			...
			if (entity instanceof Player _player) {
				ItemStack _stktoremove = new ItemStack(PalamodModItems.FAKEWTER_BUCKET.get());
