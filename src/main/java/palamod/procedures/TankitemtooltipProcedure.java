	...
	public static void execute(ItemStack itemstack) {
		...
		itemstack.set(DataComponents.CUSTOM_NAME,
				Component.literal(((((("%1" + " - " + Component.translatable("palamod.item.tank_item_l2").getString() + " - " + Component.translatable("palamod.item.tank_item_l3").getString()).replace("%4", "" + max_stock)).replace("%3",
						"" + itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("stock"))).replace("%2", itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getString("type")))
						.replace("%1", item_display))));
	}
}