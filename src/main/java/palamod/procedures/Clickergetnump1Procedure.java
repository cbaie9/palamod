...
public class Clickergetnump1Procedure {
		...
		if (clicker_buildings.exists()) {
			{
				try {
					...
					input = main_clicker_buildings.get(("building_p" + Math.round(PalamodModVariables.MapVariables.get(world).clicker_page) + "_n1")).getAsDouble();
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
			for (int index1151 = 0; index1151 < 10; index1151++) {
				if (9 >= input) {
					break;
				} else {
					input = input - 10;
