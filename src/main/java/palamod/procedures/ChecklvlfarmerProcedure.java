...
public class ChecklvlfarmerProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (jobs.exists() && !(entity instanceof Player _plr4 && _plr4.gameMode() == GameType.CREATIVE) && money.exists()) {
			{
				try {
					...
					if (main_jobs.get("next_level_farmer").getAsDouble() <= main_jobs.get("xp_farmer").getAsDouble()) {
						...
						if (entity instanceof Player _player) {
							...
							_setstack.setCount((int) main_jobs.get("lvl_farmer").getAsDouble());
							_player.getInventory().placeItemBackInInventory(_setstack);
						}
