...
public class Clickergetnump4Procedure {
		...
		if (clicker_buildings.exists()) {
			{
				try {
					...
					input = main_clicker_buildings.get(("building_p" + Math.round(PalamodModVariables.MapVariables.get(world).clicker_page) + "_n4")).getAsDouble();
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
			for (int index1154 = 0; index1154 < 10; index1154++) {
				if (9 >= input) {
					break;
				} else {
					input = input - 10;
