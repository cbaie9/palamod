...
public class Clickergetnump3Procedure {
		...
		if (clicker_buildings.exists()) {
			{
				try {
					...
					input = main_clicker_buildings.get(("building_p" + Math.round(PalamodModVariables.MapVariables.get(world).clicker_page) + "_n3")).getAsDouble();
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
			for (int index1153 = 0; index1153 < 10; index1153++) {
				if (9 >= input) {
					break;
				} else {
					input = input - 10;
