		...
		if (jobs.exists() && money.exists()) {
			{
				try {
					...
					if (main.get("next_level_alchi").getAsDouble() <= main.get("xp_alchi").getAsDouble()) {
						...
						main.addProperty("next_level_alchi", GetnextlevelxpProcedure.execute(entity));
						...
						if (world instanceof ServerLevel _level)
							_level.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, new Vec3(x, y, z), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
									("tellraw @p [\"\",{\"text\":\"[ Palamod ] :\",\"color\":\"dark_red\"},{\"text\":\" " + "" + Component.translatable("palamod.procedure.jobswinlvl_miner1").getString() + " \\n "
											+ Component.translatable("palamod.procedure.jobswinlvl_miner2").getString() + " " + Math.round(main.get("lvl_alchi").getAsDouble()) + ","
