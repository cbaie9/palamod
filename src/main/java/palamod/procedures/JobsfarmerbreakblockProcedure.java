...
public class JobsfarmerbreakblockProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (!(world instanceof ServerLevel _serverLevelGR0 && _serverLevelGR0.getGameRules().get(PalamodModGameRules.DISABLEJOBSGAMERULE.get()))) {
			...
			if (jobs.exists() && money.exists() && !(entity instanceof Player _plr10 && _plr10.gameMode() == GameType.CREATIVE)) {
				{
					try {
						...
						if (GetxpfarmerlogicProcedure.execute(world, x, y, z, entity)) {
							...
							main.addProperty("xpstreak_time_farmer", (world.getGameTime() + 80));
