...
""";
    ...
    public static boolean canCraft(LevelAccessor world, double x, double y, double z,
        ...
        // ✅ Correction : getRule().get() au lieu de getBoolean()
        if (world instanceof ServerLevel serverLevel) {
            if (serverLevel.getGameRules().get(PalamodModGameRules.DISABLEJOBSGAMERULE.get())) {
                return true;
            }
        }
        ...
        // ✅ Correction : BuiltInRegistries.ITEM.getKey()
        ...
        for (String jobName : jobData.keySet()) {
            ...
            if (playerLevel < requiredLevel) {
                MsgtellrawautosendProcedure.execute(
                        world, x, y, z,
                        "Vous devez être niveau " + requiredLevel + " en " + getJobDisplayName(jobName) + " pour crafter cet item !"
                );
                return false;
            }
        }
    ...
    public static String getRequirementText(ItemStack stack) {
        ...
        for (String jobName : jobData.keySet()) {
            ...
            result.append(requiredLevel).append(" ").append(getJobDisplayName(jobName));
        }
